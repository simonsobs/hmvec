...
class HaloModel(Cosmology):
    ...
    def add_hod(self,name,mthresh=None,ngal=None,corr="max",
        ...
        pparams = {}
        #print(self.p) ## BB
        #print(self.p['hod_Msat_override']) # BB
    ...
    def _get_hod_common(self,name):
        hod = self.hods[name]
        cname = hod['central_profile'] ## BB: none -> uc=1
        sname = hod['satellite_profile'] ## BB: nfw-> us=truncated nfw
        #uc = 1 if cname is None else self.uk_profiles[cname] ##BB: commented for consistency with class_sz
        uc = 1 # BB: uc always 1 in class_sz
    ...
    def _get_hod_square(self,name):
        ...
        # print('hod_square= ',2.*uc*us*hod['NcNs'][...,None]+hod['NsNsm1'][...,None]*us**2. )
        # print('us= ',us)
        # print('uc= ',uc)
    ...
    def _get_hod(self,name,lowklim=False):
        ...
        if lowklim:
            uc = 1
            us = 1
        #return (uc*hod['Nc'][...,None]+us*hod['Ns'][...,None])/hod['ngal'][...,None,None] #BB: formula for gg_2h
        # print('us 2h= ',us)
        # print('uc 2h= ',uc)
    ...
    def get_power_1halo(self,name="nfw",name2=None):
        ...
        if (name in hnames) and (name2 in hnames):
            #square_term = self._get_hod_square(name) # BB commented for debug
            square_term = self._get_hod_square(name) ## BB debug
            #print('square term : ', square_term)
    ...
    def get_power_2halo(self,name="nfw",name2=None,verbose=False):
        ...
        def _2haloint(iterm):
            ...
            #print('iterm 2h: ', iterm) # BB debug
            integrand = self.nzm[...,None] * iterm * self.bh[...,None] ## BB commented bias for debug
            integral = np.trapz(integrand*ms,np.log(ms),axis=-2)
            return integral

        def _get_term(iname):
            if verbose:
                print('uk_profiles: ',self.uk_profiles.keys())
                print('pk_profiles: ',self.pk_profiles.keys())
                print('hods_profiles: ',self.hods.keys())
            if iname in self.uk_profiles.keys():
                rterm1 = self._get_matter(iname)
                rterm01 = self._get_matter(iname,lowklim=False) # BB: should be True (set to false for consistency with class_sz)
                b = 1
                ...
                rterm1 = self._get_hod(iname) ## BB commented for debug
                #rterm1 = np.sqrt(self._get_hod_square(iname)) ## BB debug
        ...
        if verbose:
            ...
            print('shape int 2h: ', np.shape(integral))
            print("Two-halo integral1: " ,integral)
            print("Two-halo integral2: " ,integral2)
            # print("Two-halo b1: " ,b1)
            # print("Two-halo b2: " ,b2)
            # print("Two-halo integal-consistency1: " ,integral-consistency1)
            # print("Two-halo integal-consistency2: " ,integral2-consistency2)
        #BB: for P_mm, integral=consistency and b=1
        #return self.Pzk * (integral+b1-consistency1)*(integral2+b2-consistency2)
        return self.Pzk*(integral*integral) ## BB: consistency with class_sz
...
def ngal_from_mthresh(log10mthresh=None,zs=None,nzm=None,ms=None,
    ...
    #              Msat_override=Msat_override,
    #              Mcut_override=Mcut_override)
    # log10mstellar_thresh = np.log10(vec_unwise_mthresh(zs))
    # log10mhalo = np.log10(ms[None,:])
    # Ncs = avg_Nc(log10mhalo,zs[:,None],log10mstellar_thresh,sig_log_mstellar)
    # Nss = avg_Ns(log10mhalo,zs[:,None],log10mstellar_thresh,Ncs,
    #              sig_log_mstellar,alphasat,
...
# print('mcut at z=3:', mthresh(3))
# mthresh =  np.vectorize(mthresh)
